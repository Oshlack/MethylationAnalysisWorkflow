...
<!-- possiblePaths <- c("X:/shared/BioinfoSummer2015/450kAnalysisWorkshop", -->
<!--                   "/group/bioi1/shared/BioinfoSummer2015/450kAnalysisWorkshop", -->
<!--                   "/mnt/storage/shared/BioinfoSummer2015/450kAnalysisWorkshop") -->
<!-- if(file.exists(possiblePaths[1])){ -->
<!--     projectDirectory <- possiblePaths[1] -->
<!-- } else if(file.exists(possiblePaths[2])){ -->
<!--     projectDirectory <- possiblePaths[2] -->
<!-- } else if(file.exists(possiblePaths[3])){ -->
<!--     projectDirectory <- possiblePaths[3] -->
<!-- } -->
<!-- ``` -->
...
As for their many other BeadArray platforms, Illumina methylation data is typically obtained in the form of Intensity Data (IDAT) Files. This is a proprietary format that is output by the scanner and stores summary intensities for each probe on the array. However, there are Bioconductor packages available that facilitate the import of data from IDAT files into R [@Smith2013]. The *minfi* package provides specialised functions for importing methylation data from IDAT files, which are described in detail in the package [vignette](http://bioconductor.org/packages/release/bioc/vignettes/minfi/inst/doc/minfi.pdf). As the data for the workflow is publicly available from the Gene Expression Omnibus (GEO), a different approach will be used to import the data into R.
<!-- ```{r, eval=FALSE} -->
<!-- # set up a path for your project  -->
<!-- projectDirectory <- "/absolute/path/to/your/project" -->
<!-- ``` -->

<!-- ```{r, cache=TRUE} -->
<!-- # set up a path to your data directory - which should be in your project directory  -->
<!-- dataDirectory <- paste(projectDirectory,"data",sep="/") -->
...
All of the raw fluorescence intensities and the detection p-values required for this workflow are in the supplementary file `GSE49667_signal_intensities.txt.gz` for GEO record [GSE49667](http://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE49667). We can download them from within the R environment using the `download.file` function.    
```{r download}
dataFile <- "GSE49667_signal_intensities.txt.gz" # name of downloaded file

if(!file.exists(dataFile)){ # check if the file has already been downloaded
    # download file from GEO
    download.file("ftp://ftp.ncbi.nlm.nih.gov/geo/series/GSE49nnn/GSE49667/suppl/GSE49667_signal_intensities.txt.gz",
                  destfile = "GSE49667_signal_intensities.txt.gz")
    # read methylation data from downloaded file 
    dataTable <- read.table(file=,sep="\t", header=TRUE)
} else {
    # file already exists, read methylation data from local copy
    dataTable <- read.table(file=dataFile,sep="\t", header=TRUE)
}

head(dataTable[,1:7])
```

We will also download the sample information or **pheno** data from GEO using the `getGEO` function from the *GEOquery* package. This will create an expression set object that contains all the information related to the experiment. 
```{r get_geo}
gse <- getGEO(GEO="GSE49667")
gse
```

We can specifically extract the pheno data using the `pData` function.  
```{r get_pheno}
phenoData <- pData(gse$GSE49667_series_matrix.txt.gz)
head(phenoData)
```

As many of the fields are not relevant to the subsequent analysis, we will extract only the pertinent sample information. This can be found in the columns labelled as **characteristics**.
```{r format_pheno}
# extract only the "characteristics" columns
phenoData <- phenoData[,grepl("characteristics",colnames(phenoData))]
phenoData
```

After extracting the relevant columns, we will remove unnecessary text and update the column names with more meaningful labels.
```{r}
# remove the superfluous text
phenoData <- data.frame(sapply(phenoData, function(x) gsub(".*: ","",x,perl=TRUE)))
# label the columns with meaningful names
colnames(phenoData) <- c("Sample_Source","Sex","Disease_Status","Sample_Group")
# replace "special" characters with ordinary version
phenoData$Sample_Group <- gsub("naïve","naive",phenoData$Sample_Group)
# create a column with unique sample names
phenoData$Sample_Name <- paste(phenoData$Sample_Source,phenoData$Sample_Group,sep="_")
phenoData
```

Now that 
```{r make_mset}
samples <- gsub(".Detection.Pval", "", colnames(dataTable)[grepl("Detection", colnames(dataTable))])
cpgs <- as.character(dataTable[,1])

M <- as.matrix(dataTable[,grepl("Methylated", colnames(dataTable))]) 
U <- as.matrix(dataTable[,grepl("Unmethylated", colnames(dataTable))])
...
detP <- as.matrix(dataTable[,grepl("Detection.Pval", colnames(dataTable))])
...
```{r, cache=TRUE}
# CpG islands
islandHMM = read.csv(paste(dataDirectory, "model-based-cpg-islands-hg19.txt",
                           sep="/"),
...
dnase <- read.csv(paste(dataDirectory,"wgEncodeRegDnaseClusteredV3.bed",
                        sep="/"),
...
dnaseData <- dnaseData[seqnames(dnaseData) == chrom &
                          (start(dnaseData) >= minbase & 
                               end(dnaseData) <= maxbase)] 
dnaseData
...
cpgData <- subsetByOverlaps(cpgData, results.ranges[1])
...
```{r, echo=FALSE, results='hide', cache=TRUE}
if(grepl("group",projectDirectory)){
    source("/group/bioi1/jovanam/R_packages/gsameth.R")
    source("/group/bioi1/jovanam/R_packages/gometh.R")
    source("/group/bioi1/jovanam/R_packages/kegga.R")
} else if (grepl("X:",projectDirectory)){
    source("X:/jovanam/R_packages/gsameth.R")
    source("X:/jovanam/R_packages/gometh.R")
    source("X:/jovanam/R_packages/kegga.R")
} else if (grepl("mnt",projectDirectory)){
    source("/mnt/storage/jovanam/R_packages/gsameth.R")
    source("/mnt/storage/jovanam/R_packages/gometh.R")
    source("/mnt/storage/jovanam/R_packages/kegga.R")
}
```
...
Sample size is an important consideration when testing for differentially variable CpG sites. In order to get an accurate estimate of the group variances, larger sample sizes are required than for estimating group means. A good rule of thumb is to have at least ten samples in each group [@Phipson2014]. To demonstrate testing for differentially variable CpG sites, we will use a publicly available dataset on ageing, where whole blood samples were collected from 18 centenarians and 18 newborns and profiled for methylation on the 450k array [@Heyn2012]. We will first need to load, normalise and filter the data as previously described. 

```{r, echo=FALSE, results='hide', cache=TRUE}
possibleAgePaths <- c("X:/shared/public_data/ageing450k/Heyn",
                  "/group/bioi1/shared/public_data/ageing450k/Heyn",
                  "/mnt/storage/shared/public_data/ageing450k/Heyn")
if(file.exists(possiblePaths[1])){
    age.dataDirectory <- possibleAgePaths[1]
} else if(file.exists(possiblePaths[2])){
    age.dataDirectory <- possibleAgePaths[2]
} else if(file.exists(possiblePaths[3])){
    age.dataDirectory <- possibleAgePaths[3]
}
```

```{r, cache=TRUE, eval=FALSE, results='hide'}
# set up a path to the ageing data directory
age.dataDirectory <- "/absolute/path/to/your/ageing/data/directory"
```

```{r}
download.file("ftp://ftp.ncbi.nlm.nih.gov/geo/series/GSE30nnn/GSE30870/suppl/GSE30870_signal_intensities.txt.gz",
              destfile="GSE30870_signal_intensities.txt.gz")
```

```{r, cache=TRUE}
age.targets <- read.450k.sheet(base=age.dataDirectory)
age.targets <- age.targets[age.targets$Sample_Group != "WGBS",] 

# load the raw 450k from the IDAT files
age.rgSet <- read.450k.exp(targets=age.targets)
age.detP <- detectionP(age.rgSet) # calculate detection p-values
